...
\begin{frame}{Finite-multiplicity correction:  the fundamental problem}
  \begin{itemize}
    \item Finite-multiplicity smearing is a surjective map.
    \item An observed flow distribution may have multiple possible origin distributions.
  \end{itemize}
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[right of=l1, node distance=.3\textwidth] (r1) {$P_a(v_n^\text{obs})$};
      \node[right of=l2, node distance=.3\textwidth] (r2) {$P_b(v_n^\text{obs})$};
      \node[right of=l3, node distance=.3\textwidth] (r3) {$P_c(v_n^\text{obs})$};
...
\begin{frame}{Summary \& outlook}
  \begin{itemize}
    ...
    \item Repeat with more advanced models, especially initial conditions.
      \bgs
    \item Solve the finite-multiplicity problem.
